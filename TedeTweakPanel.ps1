...
function Set-SafePreset {
    ...
    $TxtStatus.Text = 'Preset GEAR 2 pronto da applicare.'
}
...
function Set-InsanePreset {
    ...
    Set-DebloatSelection -Items $RecommendedDebloat
    $TxtStatus.Text = 'Preset GEAR 4 pronto da applicare.'
}
...
function Set-CustomPreset {
    ...
    $TxtStatus.Text = 'Modalità GRAND LINE attiva. Seleziona i tweaks nel tab Ship Systems.'
}
...
$BtnApply.Add_Click({
    ...
    # Custom Tweak supplementari per la Tab Ship Systems
    if ($ChkGpuTweaks.IsChecked) { $selected.Add('GPU Tweaks Auto') }
    if ($ChkPagefile.IsChecked) { $selected.Add('Pagefile fisso') }
    if ($ChkWUpdateOff.IsChecked) { $selected.Add('Windows Update OFF') }
    if ($ChkETWOff.IsChecked) { $selected.Add('ETW OFF') }
    if ($ChkShaderClean.IsChecked) { $selected.Add('Shader Clean') }
    if ($ChkTimerRes.IsChecked) { $selected.Add('Timer Res') }
    if ($ChkSpectreOff.IsChecked) { $selected.Add('Spectre OFF') }
    ...
    try {
        if ($ChkServicesBase.IsChecked) { foreach ($item in (Apply-ServicesBase)) { $done.Add($item) } }
        
        if ($ChkDebloatSafe.IsChecked) { foreach ($item in (Apply-DebloatSafe)) { $done.Add($item) } }
        elseif ($ChkDebloatAggressive.IsChecked) { foreach ($item in (Apply-DebloatAggressive)) { $done.Add($item) } }
        elseif ($ChkDebloatExtreme.IsChecked) {
            $customItems = Get-SelectedDebloatItems
            if ($customItems.Count -eq 0) { $done.Add('SKIP Debloat extreme: nessuna checkbox selezionata') }
            else { foreach ($item in (Apply-DebloatSelection -Items $customItems -RemoveForUsers ([bool]$ChkDebloatUsers.IsChecked) -RemoveProvisioned ([bool]$ChkDebloatProvisioned.IsChecked))) { $done.Add($item) } }
        }

        if ($ChkPowerAdvanced.IsChecked) { foreach ($item in (Apply-PowerAdvanced)) { $done.Add($item) } }
        if ($ChkScheduler.IsChecked) { foreach ($item in (Apply-SchedulerMMCSS)) { $done.Add($item) } }
        if ($ChkInput.IsChecked) { foreach ($item in (Apply-InputTweaks)) { $done.Add($item) } }
        if ($ChkUsb.IsChecked) { foreach ($item in (Apply-UsbLowLatency)) { $done.Add($item) } }

        if ($ChkMemoryAggressive.IsChecked) { foreach ($item in (Apply-MemoryAggressive)) { $done.Add($item) } }
        elseif ($ChkMemoryLite.IsChecked) { foreach ($item in (Apply-MemoryLite)) { $done.Add($item) } }

        if ($ChkCleanupSafe.IsChecked) { foreach ($item in (Apply-BackgroundCleanupSafe)) { $done.Add($item) } }
        if ($ChkStorage.IsChecked) { foreach ($item in (Apply-StorageAdvanced)) { $done.Add($item) } }
        if ($ChkDisplay.IsChecked) { foreach ($item in (Apply-DisplayPipeline)) { $done.Add($item) } }
        if ($ChkCache.IsChecked) { foreach ($item in (Apply-CacheCleanup)) { $done.Add($item) } }
        if ($ChkCleanupPro.IsChecked) { foreach ($item in (Apply-ProcessCleanupSafePro)) { $done.Add($item) } }
        if ($ChkNetworkCommon.IsChecked) { foreach ($item in (Apply-NetworkCommon)) { $done.Add($item) } }
        ...
        if ($ChkMSI.IsChecked) { foreach ($item in (Apply-MSIMode)) { $done.Add($item) } }
        if ($ChkBCD.IsChecked) { foreach ($item in (Apply-BCDTimerTweaks)) { $done.Add($item) } }
        if ($ChkGpuVendor.IsChecked) { foreach ($item in (Apply-GpuVendorSpecific)) { $done.Add($item) } }
        
        if ($ChkGameExeGeneric.IsChecked) { foreach ($item in (Apply-GameExeGeneric -ExePath $TxtGameExePath.Text)) { $done.Add($item) } }
        ...
        if ($ChkOverlayKiller.IsChecked) { foreach ($item in (Apply-OverlayKiller)) { $done.Add($item) } }
        if ($ChkSecurityOptional.IsChecked) { foreach ($item in (Apply-SecurityOptionalProfile)) { $done.Add($item) } }
        if ($ChkVendorCleanup.IsChecked) { foreach ($item in (Apply-VendorCleanup)) { $done.Add($item) } }
        if ($ChkGaming.IsChecked) { foreach ($item in (Apply-GamingCommon)) { $done.Add($item) } }
        if ($ChkFortnite.IsChecked) { foreach ($item in (Apply-FortniteSpecific)) { $done.Add($item) } }
        
        if ($ChkGpuTweaks.IsChecked) { foreach ($item in (Apply-GpuTweaksAuto)) { $done.Add($item) } }
        if ($ChkPagefile.IsChecked) { foreach ($item in (Apply-PagefileFixed)) { $done.Add($item) } }
        if ($ChkWUpdateOff.IsChecked) { foreach ($item in (Apply-WindowsUpdateDisable)) { $done.Add($item) } }
        if ($ChkETWOff.IsChecked) { foreach ($item in (Apply-ETWOff)) { $done.Add($item) } }
        if ($ChkShaderClean.IsChecked) { foreach ($item in (Apply-ShaderCacheClean)) { $done.Add($item) } }
        if ($ChkTimerRes.IsChecked) { foreach ($item in (Set-TimerResolution)) { $done.Add($item) } }
        if ($ChkSpectreOff.IsChecked) { foreach ($item in (Apply-SpectreMitigationsOff)) { $done.Add($item) } }

        if ($ChkValidationReport.IsChecked) { $done.Add((New-TedeValidationReport)) }
        ...
        [System.Windows.MessageBox]::Show(($summary -join "`n"), 'TedeTweak - Rotta completata') | Out-Null
    }
    catch {
